...
FeedbackTargetRadiusPx = local_numeric_column(Measures, 'FeedbackTargetRadiusPx');
FeedbackDisplayRadiusPx = local_numeric_column(Measures, 'FeedbackDisplayRadiusPx');
FeedbackOuterRadiusPx = local_numeric_column(Measures, 'FeedbackOuterRadiusPx');
FeedbackDisplayType = local_text_column(Measures, 'FeedbackDisplayType', '');
FeedbackDisplayTime = local_numeric_column(Measures, 'FeedbackDisplayTime');
...
T = table(RunID, DatasetName, SubjectID, SessionID, TrialID, StrategyLabel, ...
    ...
    FeedbackValue, FeedbackTargetRadiusPx, FeedbackDisplayRadiusPx, ...
    FeedbackOuterRadiusPx, FeedbackDisplayType, FeedbackDisplayTime, ...
    IsValid, InvalidReason, DroppedChunkFlag, GapInWindowFlag, ArtifactFlag, ...
...
columns = {'RunID','DatasetName','SubjectID','SessionID','TrialID', ...
    ...
    'ZRaw','ZClipped','ZSmoothed','FeedbackValue','FeedbackTargetRadiusPx', ...
    'FeedbackDisplayRadiusPx','FeedbackOuterRadiusPx','FeedbackDisplayType', ...
    'FeedbackDisplayTime','IsValid','InvalidReason','DroppedChunkFlag', ...
    'GapInWindowFlag','ArtifactFlag','TriggerSent','AnalyticGroupDelaySamples', ...
    'EmpiricalDelaySamples','DelayCorrectionUsed','ConfigHash','BaselineConfigHash'};
end
...
textColumns = {'RunID','DatasetName','SubjectID','SessionID','TrialID', ...
    'StrategyLabel','ConditionLabel','SourceMode','FeedbackDisplayType', ...
    'InvalidReason','ConfigHash','BaselineConfigHash'};
logicalColumns = {'IsValid','DroppedChunkFlag','GapInWindowFlag', ...
    'ArtifactFlag','TriggerSent'};

T = table();
for iColumn = 1:numel(columns)
    columnName = columns{iColumn};
    if ismember(columnName, textColumns)
        T.(columnName) = emptyText;
    elseif ismember(columnName, logicalColumns)
        T.(columnName) = emptyLogical;
    else
        T.(columnName) = emptyNum;
    end
end
end
