...
assert(all(isnan(T.FeedbackTargetRadiusPx)), ...
    'Missing feedback radius fields should become NaN.');
assert(all(isnan(T.FeedbackDisplayTime)), ...
    'Missing FeedbackDisplayTime should become NaN.');
assert(all(strcmp(T.InvalidReason, '')), 'Missing text fields should become empty strings.');
assert(all(strcmp(T.FeedbackDisplayType, '')), ...
    'Missing FeedbackDisplayType should become empty strings.');
...
assert(ismember('FeedbackTargetRadiusPx', TEmpty.Properties.VariableNames), ...
    'Empty measure table is missing FeedbackTargetRadiusPx.');
