...
%% ===== CHECK OPTIONAL BRAINSTORM ENABLEMENT =====
% Explicit local runs can enable bst_function without changing defaults.
RTConfigBst = nf_brainstorm_intro_validation_config('fake_path.mat', Fs, nChannels, [8 12], ...
...
assert(strcmp(RTConfigBst.Validation.Step1.Brainstorm.Mode, 'bst_function'), ...
    'BrainstormMode override was not applied.');
assert(RTConfigBst.Validation.Step1.Brainstorm.RequireForPass == true, ...
    'RequireBrainstormForPass override was not applied.');
assert(strcmp(RTConfigBst.Brainstorm.Path, 'C:\Users\yango\Documents\brainstorm3'), ...
    'BrainstormPath override was not applied.');
assert(strcmp(RTConfigBst.Brainstorm.OfflineBandpassMethod, 'bst-hfilter-2019'), ...
    'BrainstormMethod override was not applied.');
